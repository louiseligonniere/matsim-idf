...
Returns: table of weights for each dep / sex / trage (for the specified projection scenario)
"""

def configure(context):
    context.config("data_path")
    context.config("projection_path", "projections/donnees_detaillees_departementales.zip")
    context.config("projection_scenario", None) # default = None if no projection year specified in config.yml
