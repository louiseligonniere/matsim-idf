...
"""
This stage allocates work/education locations to synthetized persons, to minimize delta with 
theoretical commute distance. 
For each origin municipality, we go through synthetized persons and choose the location 
candidate that minimizes delta between commute distance (= distance between the given person's 
home and location candidate) and theoretical distance (for the given person).
Returns: 2 dfs (work/education), assigning a location to each synthetized person (var = 
person_id, commune_id, location_id, geometry)
"""
...
def define_distance_ordering(df_persons, df_candidates, progress):
    ...
    for home_coordinate, commute_distance in zip(df_persons["home_location"], df_persons["commute_distance"]): # loop on persons
        ...
        costs[f_available] = np.abs(distances - commute_distance) # for all available locations, update costs = delta 
        # between distance to reach the given location and theoretical commute distance

        selected_index = np.argmin(costs)
        indices.append(selected_index)
        f_available[selected_index] = False # remove the selected location from availables
...
def process_municipality(context, origin_id):
    ...
    # Find relevant records (in the commune under study)
    ...
    # From previous step, this should be equal! (because we sampled as many loc candidates as persons needing one
    # for each origin commune)
...
def process(context, purpose, df_persons, df_candidates):
    ...
    with context.progress(label = "Distributing %s destinations" % purpose, total = len(df_persons)) as progress:
        with context.parallel(dict(df_persons = df_persons, df_candidates = df_candidates)) as parallel:
            for df_partial in parallel.imap_unordered(process_municipality, unique_ids): # parallelize on origin_id
...
def execute(context):
    ...
    # Separate data set (persons with work trip / persons with education trip)
    df_work = df_persons[df_persons["has_work_trip"]]
    df_education = df_persons[df_persons["has_education_trip"]]

    # Attach home locations to synthetized persons
    ...
    # Attach commute distances to synthetized persons
    ...
    # Attach geometry to work / education candidates
    ...
    # Assign destinations for work commutes
    df_work = process(context, "work", df_work, df_work_candidates)

    # Assign destinations for education commutes
